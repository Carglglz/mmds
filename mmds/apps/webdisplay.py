...
class Webdisp:
    def __init__(self, widgets):
        ...
        async def index(request):
            print(request.args)
            # return "OK", 200
            for _widget in request.args:
                widget = self.widget.get(_widget)
                if widget:
                    val = request.args.get(_widget)
                    if val[-1].isdigit():
                        val = int(val)
                    widget(val)
    ...
    async def run(self, host="0.0.0.0", port=8181):
        await self.app.start_server(host=host, port=port, ssl=self.sslctx)
...
async def gui(scr, display=None, adc=None, temp=None, dt=20, buzz=None):
    ...
    def set_temp(tmp):
        ...
        plot.update(tmp, scale=10)

    wb = Webdisp(
        {
            "text": info.set_text,
            "batt": sb.batt.set_bvalue,
            "wifi": sb.wifi.set_wvalue,
            "temp": set_temp,
        }
    )
    webtask = asyncio.create_task(wb.run())
